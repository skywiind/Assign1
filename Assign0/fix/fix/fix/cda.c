...
void insertCDA(CDA *items, int index, void *value) {
	...
	//Insert at back
		...
		//Insert left of center
			...
			items->arr[indexCDA(items, index - 1)] = value;
			items->start = (((items->start - 1) + items->cap) % items->cap);
		}
		items->size++;
	}
	return;
...
void unionCDA(CDA *recipient, CDA *donor) {
	void **temp = donor->arr;
	for (int i = donor->start; i < (donor->start + donor->size); i++) {
		insertCDAback(recipient, donor->arr[i]);
	}
	donor->size = 0;
	donor->cap = 1;
...
void freeCDA(CDA *items) {
	if (items->free != 0) {
		for (int i = indexCDA(items, 0); i < indexCDA(items, items->size); i++) {
