...
TNODE *insertBST(BST *t, void *value) {
	...
	while (node != 0) {
		...
		/*(
		int c = t->compare(value, getTNODEvalue(node));
		if (c < 0) {
...
void *findBST(BST *t, void *value) {
	...
	if (t->compare(getTNODEvalue(node), value) == 0) return getTNODEvalue(node);
	return NULL;
}
...
TNODE *locateBST(BST *t, void *value) {
	...
	if (node) {
		if (t->compare(getTNODEvalue(node), value) == 0) return node;
	}
	return NULL;
}
